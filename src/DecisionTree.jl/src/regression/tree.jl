...
module treeregressor
    ...
    export fit
    export GradientEstimator, KNNGradientEstimator, SimpleGradientEstimator, DummyGradient
    ...
    function _split!(
            ...
            gradient            :: GradientEstimator) where {S, U}
        ...
        n_constant = 0
        # true if every feature is constants
        unsplittable = true
        ...
        # We are going to reweight the sampling using the gradient
        if typeof(gradient) != DummyGradient
            mgrad = sum(map(l -> abs.(l), gradient(X[indX[(r_start+1):(r_start+n_samples)], :]))) ./ n_samples
            @inline rescale(xs::AbstractVector) = xs ./ sum(xs)
            feature_weights = (mgrad.+0.001) |> vec |> rescale |> weights
            nfeats = floor(Int, sqrt(total_features))
            if count(feature_weights .≈ 0.) > total_features - nfeats
                feature_weights = ones(eltype(feature_weights), length(feature_weights)) |> weights
            end
            feature_weights[(n_features+1):end] .= 0.
        end

        @inbounds while (unsplittable || indf <= non_constants_used) && indf <= n_features
            feature = let
                if typeof(gradient) != DummyGradient
                    sample(rng, feature_weights)
                else
    ...
    function _fit(
            ...
            gradient=DummyGradient() :: GradientEstimator) where {S, U}
